#!/usr/bin/env python3
"""Draw the brand v2 horizontal lockup: the CalCOFI mark + a "CalCOFI.io" wordmark, as outlined SVG.
...
#182B49). The wordmark reads "CalCOFI.io" (Ben, 2026-09-11): the lockup is the site's name and links to
calcofi.io, so a product header needs no second "CalCOFI.io" beside it, only its own name. The lockup is 240 units tall; the mark fills 200 of them and the capitals 150, so at the header's
...
WORDMARK = "CalCOFI.io"   # the whole string is shaped at once, so the kerning runs across "I.io"
...
def build(long_name=False):
    ...
    word_d, right = outline(FONT, WORDMARK, WGHT, size, tx, baseline)
    extra = ""
    if long_name:
    ...
    def svg(word_fill, ground):
        head = (f'<?xml version="1.0" encoding="utf-8"?>\n'
                ...
                f'     the v1 mark (its rosette paths, verbatim) + "{WORDMARK}" in Source Sans 3 wght {WGHT}, shaped by HarfBuzz and outlined,\n'
                f'     so no font is needed to draw it. {w}×{H}; use at height 36 (pages) or 28 (apps). Do not hand-edit. -->\n')
        return (head +
                f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {w} {H}" width="{w}" height="{H}" role="img" aria-label="{WORDMARK}">\n'
